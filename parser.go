...
// ParseGQLGraphPattern parses a standalone GQL graph pattern.
//
...
func (p *Parser) parseGQLPathSearchPrefix() *ast.GQLPathSearchPrefix {
	start := p.Token.Pos
	var prefix ast.GQLSearchPrefix
	var end token.Pos
	var count ast.IntValue
	...
	case p.Token.Kind == "ALL":
		...
		case p.Token.IsKeywordLike("SHORTEST"):
			prefix = ast.GQLSearchPrefixAllShortest
			end = p.Token.End
			p.nextToken()
		case p.Token.IsKeywordLike("CHEAPEST"):
			prefix = ast.GQLSearchPrefixAllCheapest
			end = p.Token.End
			p.nextToken()
		default:
	...
	case p.Token.Kind == "ANY":
		...
		case p.Token.IsKeywordLike("CHEAPEST"):
			...
			p.nextToken()
		case p.Token.Kind == token.TokenInt, p.Token.Kind == token.TokenParam, p.Token.Kind == "CAST":
			// ANY k — Spanner syntax-accepts; runtime may feature-gate.
	...
	case p.Token.IsKeywordLike("SHORTEST"):
		// SHORTEST k only (bare SHORTEST without count is a Syntax error on Spanner).
		p.nextToken()
		if p.Token.Kind != token.TokenInt && p.Token.Kind != token.TokenParam && p.Token.Kind != "CAST" {
			p.panicfAtToken(&p.Token, "bare SHORTEST path search prefix is not supported; use ANY SHORTEST or SHORTEST <count>")
		}
	...
	case p.Token.IsKeywordLike("CHEAPEST"):
		p.nextToken()
		if p.Token.Kind != token.TokenInt && p.Token.Kind != token.TokenParam && p.Token.Kind != "CAST" {
			p.panicfAtToken(&p.Token, "bare CHEAPEST path search prefix is not supported; use ANY CHEAPEST or CHEAPEST <count>")
		}
		prefix = ast.GQLSearchPrefixCheapest
		count = p.parseIntValue()
		end = count.End()
	default:
		p.panicfAtToken(&p.Token, "expected ALL, ANY, SHORTEST, or CHEAPEST, but: %s", p.Token.Kind)
	}
...
func (p *Parser) parseGQLPathMode() *ast.GQLPathModeClause {
	id := p.expect(token.TokenIdent)
	start := id.Pos
...
func (p *Parser) parseGQLEdgePattern() *ast.GQLEdgePattern {
	...
	if leftArrow && p.Token.Kind == "->" {
		p.panicfAtToken(&p.Token, "bidirectional edge pattern '<->' is not supported")
	}
...
func (p *Parser) parseGQLElementPatternFiller() *ast.GQLElementPatternFiller {
	...
	if p.Token.Kind == "IS" || p.Token.Kind == ":" {
		pos := p.Token.Pos
		is, colon := token.InvalidPos, token.InvalidPos
		if p.Token.Kind == ":" {
			colon = pos
		} else {
			is = pos
	...
	if hint == nil && variable == nil && label == nil && props == nil && where == nil && cost == nil {
		return &ast.GQLElementPatternFiller{EmptyPos: emptyPos}
	}
	return &ast.GQLElementPatternFiller{
		EmptyPos:   token.InvalidPos,
		Hint:       hint,
		Variable:   variable,
		Label:      label,
		Properties: props,
...
// cloneLexer copies the current lexer state for backtracking.
func (p *Parser) cloneLexer() *Lexer {
	// To avoid `staticcheck` warning `QF1008 - Omit embedded fields from selector expression`,
	// we need to assign `p.Lexer` to a local variable first.
	l := p.Lexer
	return l.Clone()
}
