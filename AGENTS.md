...
- `testdata/input/{ddl,dml,query,expr,gql,statement}/` — one statement per file, no trailing semicolon; `!bad_` prefix marks expected-error cases. `testdata/result/` — goldens, never hand-edit. Input basenames must be unique across ALL categories (every category is also re-run into the shared `result/statement/`).
